...
## First Tool Batch Rule

The first assistant tool-call batch must contain only the bootstrap/core load. Do not include `rg`, `git status`, `sed` on repo files, `ls`, file reads, issue lookups, screenshots, or any other inspection call in the same batch.

If the runtime cannot load skills directly, the first batch may read only this file and `../devconcept-core/SKILL.md`. Repo inspection starts after `devconcept-core` is loaded.
...
- Batching the bootstrap with `rg`, `git status`, repo file reads, or any other context gathering.
